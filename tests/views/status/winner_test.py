def test_x_won(client, create_match, create_move, create_user):
    ...
    create_move(match=match, user=match.user_x, x=0, y=0)
    create_move(match=match, user=match.user_o, x=1, y=0)
    create_move(match=match, user=match.user_x, x=0, y=1)
    create_move(match=match, user=match.user_o, x=1, y=1)
    create_move(match=match, user=match.user_x, x=0, y=2)
...
def test_o_won(client, create_match, create_move, create_user):
    ...
    create_move(match=match, user=match.user_x, x=0, y=1)
    create_move(match=match, user=match.user_o, x=1, y=1)
    create_move(match=match, user=match.user_x, x=2, y=2)
    create_move(match=match, user=match.user_o, x=1, y=2)
